...
#ifndef C__PRIMER_BLOB_2_H
#define C__PRIMER_BLOB_2_H
...
public:
    ...
    template <typename It> Blob(It b, It e) : data(make_shared<std::vector<T>>(b, e)) {}

    Blob(const Blob &orig) :
            data(std::make_shared<std::vector<T>>(*orig.data)) {}

    Blob &operator=(const Blob &right) {
        data = std::make_shared<std::vector<T>>(*right.data);
        return *this;
    }
...
template <typename T> void Blob<T>::push_back(T &&s) {
